...
function autosave_content() {
  var ed = document.getElementById("cms_content_content");
   ...
   if(wig.wysiwyg) {
     wig.theInput.value = wig.theIframe.contentWindow.document.getElementsByTagName("body")[0].innerHTML.replace("£", "&pound;");
   } else {
     wig.theInput.value = wig.theTextarea.value.replace("£", "&pound;");
   }
     $.ajax({ 
