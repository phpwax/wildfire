  ...
  jQuery(".js-added").live("click",function(){
    ...
    if(bu.hasClass('remove-button')){
      ...
      if(!checkbox_ele.hasClass('joined_field')){
        ...
        for(x in file_tags){
          tag_option_str += "<div class='clearfix'><input type='radio' name='tags["+fid+"]' value='"+file_tags[x]+"' id='inserted_tag_"+fid+"_"+x+"' class='radio_field'><label for='inserted_tag_"+fid+"_"+x+"'>"+file_tags[x]+"</label></div>";
        }
        tag_option_str += "</div>";
