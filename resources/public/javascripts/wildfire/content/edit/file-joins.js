function checkboxes_to_button(container){
  ...
  container.find("input[type=checkbox]:not(.converted-to-button)").each(function(){    
    ...
    if(jQuery('#exisiting-files .f'+obj.val()).length) obj.parent().append("<a href='#' data-fileid='"+obj.val()+"' data-input-link='"+obj.attr('id')+"' class='button js-added remove-button button-"+obj.attr('id')+"'>REMOVE</a>");
    ...
    if(bu.hasClass('remove-button')){
      ...
      if(!checkbox_ele.hasClass('joined_field')){
        var tag_option_str = "<div class='tag_options clearfix'>",            
            ord = ord = jQuery('#exisiting-files .joined-file').length,
            order_str = '<input type="hidden" name="tags['+fid+'][order]" value="'+ord+'" class="join-order-field">'
            ;
        for(x in file_tags){
        ...
        tag_option_str += "</div>";
        insert_str = '<div class="joined-file clearfix f'+fid+'">'+order_str+'<img src="'+jQuery(imgs).attr('src')+'" alt="'+jQuery(imgs).attr('alt')+'">'+tag_option_str+'<a id="button-joined_file_'+fid+'" class="button js-added remove-button" data-input-link="join_file_'+fid+'" data-fileid="'+fid+'" href="#">REMOVE</a></div>';
        jQuery('#exisiting-files').append(insert_str);
        checkbox_ele.clone().attr("checked","checked").appendTo("#exisiting-files .f"+fid);
      }
