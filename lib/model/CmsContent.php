...
class CmsContent extends WaxModel {
	...
	public function before_save() {
	  ...
	  if(!$this->expires || $this->expires == $this->published) $this->expires = date("Y-m-d H:i:s", strtotime("-10 years"));
	}
	...
	public function permalink() {
	  if(!$this->url || is_numeric($this->url)) $this->generate_url();
  ...
  public function scope_published() {
    ...
    if(Config::get('use_expiry_date')){
      $this->filter("(`expires` <=  `published` OR (`expires` >=  `published` AND `expires` >= NOW() ))");
    }
