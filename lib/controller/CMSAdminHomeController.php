  ...
  public function visitor_data() {
    if($this->current_user->access($this->module_name,"stats")){
      ...
      if($api->login($analytics_email, $analytics_password)) {
        ...
        $chart->add_x_axis(array("labels"=>array("labels"=>array_reverse($labels) ,"colour"=>"#666666","size"=>9),"colour"=>"#D1D1D1","grid-colour"=>"#d1d1d1","stroke"=>1,"font-size"=>9  ));
        $chart->add_y_axis(array("labels"=>array("colour"=>"#666666","size"=>9),"stroke"=>1,"font-size"=>9,"colour"=>"#D1D1D1","grid-colour"=>"#d1d1d1","min"=>0,"max"=>max($raw_visits)+10,"steps"=>ceil(max($raw_visits)/100)*10));
