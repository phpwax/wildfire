...
class CMSAdminHomeController extends AdminComponent {
  ...
  public function visitor_data() {
    ...
    if($api->login(Config::get("analytics/email"), Config::get("analytics/password"))) {
      ...
      $chart->add_x_axis(array("labels"=>array("labels"=>array_reverse($labels) ),"colour"=>"#666666","grid-colour"=>"#D1D1D1","stroke"=>1,"font-size"=>9  ));
      $chart->add_y_axis(array("stroke"=>1,"font-size"=>9,"colour"=>"#666666","grid-colour"=>"#D1D1D1","min"=>0,"max"=>max($raw_visits)+10,"steps"=>ceil(max($raw_visits)/100)*10));
