...
class CMSApplicationController extends WaxController{
     ...
     */
    public function cms_view($stack = [], $language_id = null)
    {
        $accumulated = "";
        ...
        //if there is one thing in the stack, and it is an allowed language, push that to the stack
        else if(count($stack)===1 && ($key = array_shift(array_keys($stack))) && CMSApplication::$languages[$language_id] && CMSApplication::$languages[$language_id]['url'] == $stack[$key]) {
            $stack[] = "home";
        }
        foreach($stack as $item){
            $accumulated .= $item."_";
        ...
        foreach(array_reverse($views) as $view){
            if(is_array($view) && $this->is_viewable($view['path'], $this->use_format, $view['plugin'])) {
                return basename($view['path']);
            }
            else if(!is_array($view) && $this->is_viewable($view, $this->use_format)) {
                return $view;
            }
        }
        return false;
    ...
    public function map_to_content($map){
        if($map->destination_url && !$map->track_url) $this->redirect_to($map->destination_url, "http://", $map->header_status);
